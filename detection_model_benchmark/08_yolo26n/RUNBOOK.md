...
Dataset format is YOLO. Finalize `dataset_config.yaml` from the example. Weight: `weights/yolo26n.pt`; `PYTHONPATH` pins the cloned v8.4.98 source.

From `src/`:

```bash
PYTHONPATH="$PWD" yolo detect train model=../weights/yolo26n.pt data=../dataset_config.yaml imgsz="$IMG_SIZE" batch="$BATCH_SIZE" epochs="$EPOCHS" device="$DEVICE" project="$OUTPUT_ROOT/08_yolo26n" name=train
