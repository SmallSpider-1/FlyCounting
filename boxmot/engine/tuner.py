...
from boxmot.engine.evaluator import eval_init, run_generate_dets_embs, run_generate_mot_results, run_trackeval
    ...
    def objective_function(self, config: dict) -> dict:
        ...
        if values and all(isinstance(v, dict) for v in values):
            return {
                k: max(
                    0,
                    sum(c.get(k, 0) for c in results.values() if isinstance(c, dict)) / max(1, len(results)),
                )
