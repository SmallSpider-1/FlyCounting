...
from boxmot.utils import DATASET_CONFIGS, ROOT, TRACKEVAL, WEIGHTS
...
def core_options(func):
    options = [
        click.option("--source", type=str, default="0", help="file/dir/URL/glob, 0 for webcam"),
        click.option(
            "--imgsz",
        ...
        click.option("--fps", type=int, default=30, help="video frame-rate"),
        click.option("--conf", type=float, default=0.01, help="min confidence threshold"),
        click.option("--iou", type=float, default=0.7, help="IoU threshold for NMS"),
        click.option("--device", default="", help="cuda device(s), e.g. 0 or 0,1,2,3 or cpu"),
        click.option(
            "--batch-size",
            type=int,
            default=16,
        ...
        click.option("--project", type=Path, default=ROOT / "runs", help="save results to project/name"),
        click.option("--name", default="", help="save results to project/name"),
        click.option("--exist-ok", is_flag=True, default=True, help="existing project/name ok, do not increment"),
        click.option("--half", is_flag=True, help="use FP16 half-precision inference"),
        click.option("--vid-stride", type=int, default=1, help="video frame-rate stride"),
        click.option("--ci", is_flag=True, help="reuse existing runs in CI (no UI)"),
        click.option("--tracking-method", type=str, default="deepocsort", help="deepocsort, botsort, strongsort, ..."),
        click.option("--verbose", is_flag=True, help="print detailed logs"),
        click.option("--agnostic-nms", is_flag=True, help="class-agnostic NMS"),
        click.option(
            "--postprocessing",
            type=click.Choice(["none", "gsi", "gbrc"], case_sensitive=False),
            default="none",
            help="Postprocess tracker output: none | gsi (Gaussian smoothed interpolation) | gbrc (gradient boosting smooth).",
        ),
        click.option("--show", is_flag=True, help="display tracking in a window"),
        click.option("--show-labels/--hide-labels", default=True, help="show or hide detection labels"),
        click.option("--show-conf/--hide-conf", default=True, help="show or hide detection confidences"),
        click.option("--show-trajectories", is_flag=True, help="overlay past trajectories"),
        click.option("--show-lost", is_flag=True, help="show lost tracks"),
        click.option("--save-txt", is_flag=True, help="save results to a .txt file"),
        click.option("--save-crop", is_flag=True, help="save cropped detections"),
        click.option("--save", is_flag=True, help="save annotated video"),
        click.option("--line-width", type=int, help="bounding box line width"),
        click.option("--per-class", is_flag=True, help="track each class separately"),
        click.option("--target-id", type=int, default=None, help="ID to highlight in green"),
    ]
...
def parse_classes(classes_input):
    """Parse classes input which can be a tuple of ints (from multiple=True), a string (comma/space separated), or None.
...
def singular_model_options(func):
    options = [
        click.option(
            "--yolo-model", type=Path, default=WEIGHTS / "yolov8n.pt", help="path to YOLO weights for detection"
        ),
        click.option(
            "--reid-model", type=Path, default=WEIGHTS / "osnet_x0_25_msmt17.pt", help="path to ReID model weights"
        ),
...
def export_options(func):
    ...
    options = [
        ...
        click.option("--device", default="cpu", help="CUDA device (e.g., '0', '0,1,2,3', or 'cpu')"),
        click.option("--optimize", is_flag=True, help="Optimize TorchScript for mobile (CPU export only)"),
        click.option("--dynamic", is_flag=True, help="Enable dynamic axes for ONNX/TF/TensorRT export"),
        click.option("--simplify", is_flag=True, help="Simplify ONNX model"),
        click.option("--opset", type=int, default=18, help="ONNX opset version"),
        click.option("--workspace", type=int, default=4, help="TensorRT workspace size (GB)"),
        ...
        click.option("--half", is_flag=True, help="Enable FP16 half-precision export (GPU only)"),
        click.option(
            "--include",
            multiple=True,
            default=("torchscript",),
...
def tune_options(func):
    """Decorator adding ReID export options (ported from argparse export script)."""
    options = [
        click.option("--n-trials", type=int, default=4, help="number of trials for evolutionary tuning"),
        click.option(
            "--objectives",
            type=str,
            multiple=True,
    ...
    def format_help(self, ctx, formatter):
        ...
        with formatter.indentation():
            ...
            formatter.write_text(
                "       TRACKER (optional) is one of [deepocsort, botsort, bytetrack, strongsort, ocsort, hybridsort]"
            )
            formatter.write_text(
                "       ARGS (optional) 'arg=value' pairs like 'source=0' 'imgsz=640' that override defaults."
            )
            formatter.write_text(
                "          See all ARGS at https://github.com/mikel-brostrom/boxmot or 'boxmot MODE --help'"
            )
        formatter.write_paragraph()
...
def export(ctx, **kwargs):
    """Command 'export': export ReID model weights and configurations for deployment. Mirrors the standalone
    argparse-based export script.
    """
