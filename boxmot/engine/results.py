...
class Tracks:
    ...
    def _default_draw(self, frame, tracks):
        ...
        annotator = Annotator(frame, line_width=2, example=" ")
        for t in tracks:
            bbox = t[:4]
            id = int(t[4])
            conf = t[5]
...
class Results:
    def __init__(self, source: str | int | Path, detector: Any, reid: Any, tracker: Any, verbose: bool = True):
        self.source = source
        ...
        # Timing accumulators
        self.totals = {"det": 0.0, "reid": 0.0, "track": 0.0, "total": 0.0, "frames": 0}
