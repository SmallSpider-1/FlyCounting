...
def resolve_yolo_model_path(yolo_model_path: str | Path) -> Path:
...
class TimedReIDModel:
    ...
    This wrapper intercepts calls to get_features() and records the time spent in ReID feature extraction.
    """

    def __init__(self, model, timing_stats: TimingStats | None = None):
...
class DetectorReIDPipeline:
    ...
    This class provides a consistent interface for running YOLO detection and ReID feature extraction across both
    real-time tracking and batch evaluation. It handles:
    ...
    def __init__(
        ...
        reid_model_paths: str | Path | list[str | Path] | None = None,
        device: str = "",
    ...
    def _init_reid_models(self, reid_model_paths: str | Path | list[str | Path]):
        """Initialize ReID models with timing wrappers.
...
def extract_detections(result) -> np.ndarray:
    ...
    Returns:
        numpy array of shape (N, 6) for AABB models with columns: [x1, y1, x2, y2, conf, cls], or shape (N, 7) for OBB
            models with columns [cx, cy, w, h, angle, conf, cls]. Returns an empty array with the correct width when no
