...
def on_predict_start(predictor, args, timing_stats=None):
    ...
    """
    assert args.tracking_method in TRACKERS, f"'{args.tracking_method}' is not supported. Supported ones are {TRACKERS}"
...
def plot_trajectories(predictor, timing_stats=None, video_writer=None):
    ...
    for i, result in enumerate(predictor.results):
        ...
        result.orig_img = tracker.plot_results(
            img, predictor.custom_args.show_trajectories, show_lost=predictor.custom_args.show_lost
        )
...
def main(args):
    ...
    pipeline.add_callback(
        "on_predict_postprocess_end", partial(plot_trajectories, timing_stats=timing_stats, video_writer=video_writer)
    )
