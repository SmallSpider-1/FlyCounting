...
def linear_interpolation(data: np.ndarray, interval: int) -> np.ndarray:
    ...
    The function assumes the first two columns of `data` represent frame number and object ID. Interpolated rows are
    added when consecutive rows for the same ID have a gap of more than 1 frame but less than the specified interval.
...
def gaussian_smooth(data: np.ndarray, tau: float) -> np.ndarray:
    ...
    For each unique object ID in the data, this function smooths columns 2 through 5 using a Gaussian Process with an
    RBF kernel. Additional columns (columns 6 and 7) and a constant value (-1) are appended to each row.
