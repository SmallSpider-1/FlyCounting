...
def ct_dist(bboxes1, bboxes2):
    """Measure the center distance between two sets of bounding boxes, this is a coarse implementation, we don't
    recommend using it only for association, which can be unstable and sensitive to frame rate and object speed.
    """
    bboxes2 = np.expand_dims(bboxes2, 0)
    bboxes1 = np.expand_dims(bboxes1, 1)
...
def linear_assignment(cost_matrix, thresh=0.0):
...
def associate_detections_to_trackers(detections, trackers, iou_threshold=0.3):
    """Assigns detections to tracked object (both represented as bounding boxes) Returns 3 lists of matches,
    unmatched_detections and unmatched_trackers.
    """
    if len(trackers) == 0:
...
def associate_4_points(
    ...
    _YC, _XC = speed_direction_batch(detections, previous_obs)
        ...
        if emb_cost is None:
            ...
            if not with_longterm_reid:
                ...
                matched_indices = linear_assignment(
                    ...
                    + weights[1] * emb_cost
                    + longterm_reid_weight * long_emb_dists
                )  # , thresh=thresh
...
def embedding_distance(tracks_feat, detections_feat, metric="cosine"):
    ...
    cost_matrix = np.maximum(
        0.0, cdist(tracks_feat, detections_feat, metric)
    )  # Normalized features, metric: cosine, [track_num, detection_num]
    return cost_matrix
