...
class TimingStats:
    ...
    def print_summary(self):
        ...
        log("<blue>" + "=" * 105 + "</blue>")
        log(
            f"<bold>{'Component':<20}</bold> | {'Total (ms)':<12} | {'Avg (ms)':<12} | {'FPS':<10} | {'% of Total':<12}"
        )
        log("<blue>" + "-" * 105 + "</blue>")
        ...
        for key in ["preprocess", "inference", "postprocess"]:
            ...
            log(
                f"{key.capitalize():<20} | <blue>{total:<12.1f}</blue> | <blue>{avg:<12.2f}</blue> | <blue>{fps:<10.1f}</blue> | {pct(total):<12.1f}"
            )

        det_avg = det_total / frames
        det_fps = fps_from_avg(det_avg)
        log(
            f"<bold>{'Detection (total)':<20}</bold> | <cyan>{det_total:<12.1f}</cyan> | <cyan>{det_avg:<12.2f}</cyan> | <cyan>{det_fps:<10.1f}</cyan> | {pct(det_total):<12.1f}"
        )
        ...
        log(
            f"{'ReID':<20} | <blue>{reid_total:<12.1f}</blue> | <blue>{reid_avg:<12.2f}</blue> | <blue>{reid_fps:<10.1f}</blue> | {pct(reid_total):<12.1f}"
        )
        ...
        if track_total > 0:
            ...
            log(
                f"{'Association':<20} | <blue>{assoc_time:<12.1f}</blue> | <blue>{assoc_avg:<12.2f}</blue> | <blue>{assoc_fps:<10.1f}</blue> | {pct(assoc_time):<12.1f}"
            )
        ...
        if plot_time > 0:
            ...
            log(
                f"{'Plotting':<20} | <blue>{plot_time:<12.1f}</blue> | <blue>{plot_avg:<12.2f}</blue> | <blue>{plot_fps:<10.1f}</blue> | {pct(plot_time):<12.1f}"
            )
        ...
        if overhead > 0:
            ...
            log(
                f"{'Other (I/O, etc)':<20} | <blue>{overhead:<12.1f}</blue> | <blue>{overhead_avg:<12.2f}</blue> | <blue>{overhead_fps:<10.1f}</blue> | {pct(overhead):<12.1f}"
            )
        ...
        log(
            f"<bold>{'Total':<20}</bold> | <cyan>{total_time:<12.1f}</cyan> | <cyan>{avg_total:<12.2f}</cyan> | <cyan>{total_fps:<10.1f}</cyan> | {100.0:<12.1f}"
        )
