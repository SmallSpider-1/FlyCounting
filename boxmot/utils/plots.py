...
class MetricsPlotter:
    """A class for plotting tracking metrics in both radar and FPS-vs-metric line charts. Instead of displaying the
    plots, this version saves them to disk in a specified root folder.

    Attributes:
        ----------
        root_folder: str The directory where both plots will be saved.

    Methods:
        -------: plot_radar_chart(data, labels, title='Radar Chart', figsize=(6, 6), ylim=(0, 100.0), yticks=None,
            ytick_labels=None, filename='radar_chart.png') Plots a radar chart for multiple methods over a fixed set of
            metrics, then saves it into the root folder under `filename`.
    ...
    def plot_radar_chart(
        ...
        for method, values in data.items():
            if len(values) != num_vars:
                raise ValueError(
                    f"Length of values for '{method}' ({len(values)}) does not match number of labels ({num_vars})."
                )
...
if __name__ == "__main__":
    ...
    plotter.plot_fps_metrics(fps, fps_data, title="YOLOX-X + BoostTrack MOT17 metrics vs FPS", figsize=(12, 6))
