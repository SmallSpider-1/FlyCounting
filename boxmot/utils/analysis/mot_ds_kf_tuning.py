...
def load_gt_data(seq_dir: Path, annotations_dir: Path | None = None, use_temp_gt: bool = False):
...
def build_tracks_from_sequence(
    ...
    """Load ground-truth from a single MOT sequence folder, build 8-D state/4-D measurement tracks for each object, and
    ...
    for obj_id in np.unique(orig_gt[:, 1].astype(int)):
        ...
        # centers
        ctrs = np.stack([sel[:, 2] + sel[:, 4] / 2, sel[:, 3] + sel[:, 5] / 2], axis=1)
        # widths/heights
        wh = sel[:, 4:6]
        # velocities
    ...
    for seq_dir in sorted(seq_root.iterdir()):
        ...
        try:
            tracks, ws, hs = build_tracks_from_sequence(
                seq_dir, annotations_dir=annotations_dir, use_temp_gt=use_temp_gt, min_detections=min_detections
            )
...
""",
    ...
    parser.add_argument("--use_temp_gt", action="store_true", help="Use gt_temp.txt instead of gt.txt")
    parser.add_argument("--min_detections", type=int, default=5, help="Minimum detections per track to include")
