...
def plot_gt_boxes_with_trajectories(seq_dir: str | Path, use_temp_gt: bool = True, pad: int = 0):
    """Plot all ground-truth boxes for a MOT17 sequence, coloring each object ID with a unique color and drawing its
    trajectory through the centers of its boxes.

    Args:
        seq_dir (str | Path): Path to the sequence folder (containing img1/ and gt/).
        use_temp_gt (bool): If True, plot gt/gt_temp.txt (filtered by FPS). Otherwise plot gt/gt.txt.
    ...
    _fig, ax = plt.subplots(figsize=(10, 10 * (ymax - ymin) / (xmax - xmin)))
    ...
    for row in orig_gt:
        _frame, obj_id, x, y, w_box, h_box = row[:6]
        obj_id = int(obj_id)
