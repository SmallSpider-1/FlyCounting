...
> 协议版本：v2.1（v2.0 尚未冻结 `protocol_sha256`，本次为冻结前的原地修订：新增缓存写入精度策略，其余研究设计未变）
> 更新日期：2026-07-27
...
检测端已于 2026-07-27 完成：独立 RT-DETR-R18 入口强制加载 checkpoint 的 `ema.module`，使用 FP32、直接拉伸 640×640、`orig_size=(w,h)` 和原生 NMS-free top-300 postprocessor，并保存两个类别中 `confidence ≥ 0.10` 的检测；`--iou` 不被接受，缓存元数据明确记录 `nms_iou=not_applicable`。

冻结 val 的 1,530 张图、430,357 行预测逐框逐值完全一致（坐标、confidence、class 和顺序最大误差均为 0）。25 个片段按 manifest 的 `video_id + segment_index` 显式读取并完成缓存：实际解码 10,007,059 帧、空帧不跳过、25/25 footer 完整，集合 numeric payload SHA-256 为 `83d60d1e0ce66d77f948c4b3072426bba66faccb55fe224ec87ef68a8b64bd18`。检测阶段完成不等于 8 tracker 与计数实验已经运行。
...
```text
x1, y1, x2, y2   保留 2 位小数
confidence       保留 4 位小数
class_id         整数（非整数值直接拒绝写入）
```

四舍五入固定为“远离零”，不使用 Python 默认的银行家进位。该策略写入缓存头并在读取时校验，因此精度本身是受控变量而不是依赖 NumPy 版本的隐式行为。

需要注意其边界：截断只能吸收同一四舍五入区间内的浮点尾数差异，若两次推理的数值正好落在 `.xx5` 分界线两侧仍会写出不同结果。因此不能据此声称跨环境 payload 哈希必然一致。10.7 要求的“同一检测缓存与配置重复运行两次哈希一致”依赖的是同一环境下推理逐位确定，与本精度策略无关。
...
轨迹缓存的 `track_id`、`class_id`、`detection_index` 按整数写入，坐标与置信度沿用 9.3 的精度策略。`detection_index = -1` 表示元数据回填失败，属于合法取值。
...
- [ ] top-300；
- [ ] cache floor 0.10；
- [ ] 缓存写入精度策略为 `cache_version=2`（坐标 2 位、置信度 4 位、ID 整数）；
- [ ] 全画面坐标恢复验证通过；
...
若旧文档与本协议在以下两点冲突，以本协议 v2.1 和用户 2026-07-25 最新决定为准：
