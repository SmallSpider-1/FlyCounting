...
class FreqFusion(nn.Module):
    ...
    def _forward(self, hr_feat, lr_feat):
        ...
        if self.semi_conv:
            ...
            compressed_x = (
                F.interpolate(compressed_lr_feat, size=compressed_hr_feat.shape[-2:], mode="nearest")
                + compressed_hr_feat
            )
        ...
        if self.use_high_pass:
            ...
            if self.hr_residual:
                ...
                hr_feat_hf = hr_feat - carafe(
                    hr_feat, mask_hr.to(compressed_hr_feat.dtype), self.highpass_kernel, self.up_group, 1
                )
...
def compute_similarity(input_tensor, k=3, dilation=1, sim="cos"):
    ...
    if sim == "cos":
        similarity = F.cosine_similarity(
            unfold_tensor[:, :, k * k // 2 : k * k // 2 + 1], unfold_tensor[:, :, :], dim=1
        )
    elif sim == "dot":
