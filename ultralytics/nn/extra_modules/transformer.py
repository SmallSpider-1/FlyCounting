...
class DPB_Attention(nn.Module):
    ...
    def __init__(
        self, dim, group_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0.0, proj_drop=0.0, position_bias=True
    ):
...
class DPBlock(PSABlock):
    ...
    def forward(self, x):
        ...
        _BS, C, H, W = x.size()
        x = (
            x + self.attn(x.flatten(2).permute(0, 2, 1)).permute(0, 2, 1).view([-1, C, H, W]).contiguous()
            if self.add
            else self.attn(x.flatten(2).permute(0, 2, 1)).permute(0, 2, 1).view([-1, C, H, W]).contiguous()
        )
...
class ASSAlock(PSABlock):
    ...
    def forward(self, x):
        ...
        _BS, C, H, W = x.size()
        x = (
            x + self.attn(x).permute(0, 2, 1).view([-1, C, H, W]).contiguous()
            if self.add
            else self.attn(x).permute(0, 2, 1).view([-1, C, H, W]).contiguous()
        )
        x = x + self.ffn(x) if self.add else self.ffn(x)
        return x
...
class C2TSSA_DYT_Mona_EDFFN(C2PSA):
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.m = nn.Sequential(
            *(TSSAlock_DYT_Mona_EDFFN(self.c, attn_ratio=0.5, num_heads=self.c // 64) for _ in range(n))
        )
