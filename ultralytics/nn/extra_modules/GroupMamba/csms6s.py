...
class SelectiveScanOflex(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.delta_softplus = delta_softplus
        out, x, *_rest = selective_scan_cuda_oflex.fwd(u, delta, A, B, C, D, delta_bias, delta_softplus, 1, oflex)
        ctx.save_for_backward(u, delta, A, B, C, D, delta_bias, x)
        return out
    ...
    def backward(ctx, doubt, *args):
        ...
        if doubt.stride(-1) != 1:
            doubt = doubt.contiguous()
        du, ddelta, dA, dB, dC, dD, ddelta_bias, *_rest = selective_scan_cuda_oflex.bwd(
            u, delta, A, B, C, D, delta_bias, doubt, x, ctx.delta_softplus, 1
        )
        return (du, ddelta, dA, dB, dC, dD, ddelta_bias, None, None, None, None)
