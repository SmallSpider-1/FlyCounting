...
class BaseRouter(nn.Module):
    ...
    def _process_logits(
        self, logits: torch.Tensor, noise_std: float, training: bool
...
class AdvancedRoutingLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if (
            hasattr(self, "router")
            and isinstance(self.router, nn.Sequential)
            and len(self.router) > 0
            and isinstance(self.router[0], nn.Conv2d)
        ):
            expected_in = self.router[0].in_channels
            if expected_in != C:
                if (
                    not hasattr(self, "_proj")
                    or not isinstance(self._proj, nn.Conv2d)
                    or self._proj.in_channels != C
                    or self._proj.out_channels != expected_in
                ):
