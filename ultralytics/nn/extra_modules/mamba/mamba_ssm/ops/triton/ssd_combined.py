...
from mamba_ssm.ops.triton.layernorm_gated import _layer_norm_bwd, _layer_norm_fwd, rmsnorm_fn
...
def _chunk_scan_chunk_state_bwd_dx_kernel(
    ...
    for k in range(K_MIN, K_MAX, BLOCK_SIZE_K):
        ...
        doubt = tl.load(dout_ptrs, mask=(offs_k[:, None] < K_MAX - k) & (offs_n[None, :] < hdim), other=0.0)
...
def _chunk_scan_chunk_state_bwd_dx(x, dt, dA_cumsum, B, CB, doubt, dstates, D=None, seq_idx=None, dx=None):
    ...
    ddt = torch.empty(batch, nheads, nchunks, chunk_size, device=doubt.device, dtype=torch.float32)
...
def _mamba_chunk_scan_combined_bwd(
    ...
    _, _, ngroups, dstate = B.shape
    assert doubt.shape == (batch, seqlen, nheads, headdim)
    assert dt.shape == (batch, seqlen, nheads)
    assert A.shape == (nheads,)
    assert nheads % ngroups == 0
    assert B.shape == (batch, seqlen, ngroups, dstate)
    ...
    dt_in = dt.clone()
    dA_cumsum, dt = _chunk_cumsum_fwd(dt_in, A, chunk_size, dt_bias=dt_bias, dt_softplus=dt_softplus, dt_limit=dt_limit)
    CB = _bmm_chunk_fwd(C, B, chunk_size, seq_idx=seq_idx, output_dtype=torch.float32)
    states = _chunk_state_fwd(B, x, dt, dA_cumsum, seq_idx=seq_idx, states_in_fp32=True)
    ...
    if z is not None:
        dz, doubt, dD, *rest = _chunk_scan_bwd_dz(
            x, z, out, doubt, chunk_size=chunk_size, has_ddAcs=False, D=D, dz=dz, recompute_output=recompute_output
        )
        outz = rest[0] if recompute_output else out
    else:
        dz = None
        outz = out
    dstates = _chunk_scan_bwd_dstates(C, dA_cumsum, doubt, seq_idx=seq_idx, dtype=states.dtype)
    ...
    dx, ddt, dD_from_x = _chunk_scan_chunk_state_bwd_dx(
        x, dt, dA_cumsum, B, CB, doubt, dstates, D=D, seq_idx=seq_idx, dx=dx
    )
    ...
    # dC = _chunk_scan_bwd_dC(states[:, :-1].to(x.dtype), dA_cumsum, doubt, seq_idx=seq_idx, ngroups=ngroups)
    dC, ddA_cumsum_prev = _chunk_scan_bwd_dC(states.to(x.dtype), dA_cumsum, doubt, seq_idx=seq_idx, C=C, ngroups=ngroups)
    # Computing ddA with the dcb kernel is much slower, so we're not using it for now
    dCB = _chunk_scan_bwd_dcb(x, dt, dA_cumsum, doubt, seq_idx=seq_idx, ngroups=ngroups)
    # dCB, ddA_tmp = _chunk_scan_bwd_dcb(x, dt, dA_cumsum, doubt, seq_idx=seq_idx, CB=CB, ngroups=ngroups)
    dCB = dCB.to(CB.dtype)
    ...
    # ddA_cumsum = torch.einsum("bclhp,bclhp->bhcl", out.float(), doubt.float()) - ddt * dt
    ...
    # ddA_cumsum_prev = _chunk_scan_bwd_ddAcs_prev(states[:, :-1], C, doubt, dA_cumsum, seq_idx=seq_idx)
    ...
    ddA = _chunk_scan_bwd_ddAcs_stable(x, dt, dA_cumsum, doubt, CB)
    ...
    # _, dA = selective_scan_bwd(doubt, x, dt, A, B, C, D=D.float(), z=z)
    # ddt_given.copy_(ddt)
...
def selective_scan_bwd(doubt, x, dt, A, B, C, D=None, z=None):
    """Argument: doubt: (batch, seqlen, nheads, headdim) x: (batch, seqlen, nheads, headdim) dt: (batch, nheads, nchunks,
    chunk_size) or (batch, nheads, headdim, nchunks, chunk_size) A: (nheads) or (dim, dstate) B: (batch, seqlen,
    ngroups, dstate) C: (batch, seqlen, ngroups, dstate) D: (nheads, headdim) or (nheads,) z: (batch, seqlen,
    nheads, headdim).
    ...
    _batch, _seqlen, nheads, headdim = x.shape
    ...
    doubt = rearrange(doubt, "b l h p -> b (h p) l")
...
class MambaChunkScanCombinedFn(torch.autograd.Function):
    ...
    def forward(
        ...
        out, out_x, _dt_out, dA_cumsum, _states, final_states = _mamba_chunk_scan_combined_fwd(
            x,
            dt,
            A,
            B,
    ...
    def backward(ctx, doubt, *args):
        out, x, dt, _dA_cumsum, A, B, C, D, z, dt_bias, initial_states, seq_idx = ctx.saved_tensors
...
def mamba_chunk_scan_combined(
    ...
    """Argument: x: (batch, seqlen, nheads, headdim) dt: (batch, seqlen, nheads) A: (nheads) B: (batch, seqlen, ngroups,
    dstate) C: (batch, seqlen, ngroups, dstate) chunk_size: int D: (nheads, headdim) or (nheads,) z: (batch, seqlen,
    nheads, headdim) dt_bias: (nheads,) initial_states: (batch, nheads, headdim, dstate) seq_idx: (batch, seqlen)
...
def mamba_chunk_scan(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, dt_softplus=False):
    """Argument: x: (batch, seqlen, nheads, headdim) dt: (batch, seqlen, nheads) A: (nheads) B: (batch, seqlen, ngroups,
    dstate) C: (batch, seqlen, ngroups, dstate) D: (nheads, headdim) or (nheads,) z: (batch, seqlen, nheads,
    headdim) dt_bias: (nheads,).

    Returns:
        out: (batch, seqlen, nheads, headdim).
    """
    _batch, seqlen, _nheads, _headdim = x.shape
    dstate = B.shape[-1]
...
def ssd_selective_scan(x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    """Argument: x: (batch, seqlen, nheads, headdim) dt: (batch, seqlen, nheads) or (batch, seqlen, nheads, headdim) A:
    (nheads) or (dim, dstate) B: (batch, seqlen, ngroups, dstate) C: (batch, seqlen, ngroups, dstate) D: (nheads,
    headdim) or (nheads,) z: (batch, seqlen, nheads, headdim) dt_bias: (nheads,) or (nheads, headdim).
    ...
    _batch, _seqlen, _nheads, headdim = x.shape
    _, _, _ngroups, dstate = B.shape
    x = rearrange(x, "b l h p -> b (h p) l")
    if dt.dim() == 3:
...
def mamba_conv1d_scan_ref(
    ...
    """Argument: xBC: (batch, seqlen, dim + 2 * ngroups * dstate) where dim == nheads * headdim conv1d_weight: (dim + 2
    * ngroups * dstate, width) conv1d_bias: (dim + 2 * ngroups * dstate,) dt: (batch, seqlen, nheads) or (batch,
    seqlen, nheads, headdim) A: (nheads) D: (nheads, headdim) or (nheads,) z: (batch, seqlen, dim) dt_bias: (nheads)
    or (nheads, headdim) headdim: if D is 1D and z is None, headdim must be passed in.

    Returns:
        out: (batch, seqlen, dim).
    """
    _batch, _seqlen, nheads = dt.shape[:3]
    assert nheads % ngroups == 0
    if z is not None:
        dim = z.shape[-1]
        assert dim % nheads == 0
...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def forward(
        ...
        if rmsnorm_weight is None:
            ...
            out_x, _, _dt_out, _dA_cumsum, _states, final_states = _mamba_chunk_scan_combined_fwd(
                x,
                dt,
                A,
                B,
    ...
    def backward(ctx, doubt, *args):
        ...
        if outproj_weight is not None:
            dout_og = doubt
            doubt = F.linear(doubt, outproj_weight.t())
        if d_nonssm > 0:
            dout0, doubt = doubt.split([d_nonssm, dim], dim=-1)
            _swiglu_bwd(zx0, dout0, dxy=dzx0, recompute_output=True, out=out0_recompute)
        doubt = rearrange(doubt, "b s (h p) -> b s h p", p=headdim)
        if rmsnorm_weight is None:
            ...
            batch = doubt.shape[0]
            dy_rms = rearrange(doubt, "b s h p -> (b s) (h p)")
            ...
            doubt, drmsnorm_weight, _, dz, *rest = _layer_norm_bwd(
                dy_rms,
                x_rms,
                rmsnorm_weight,
                None,
            ...
            doubt = rearrange(doubt, "(b s) (h p) -> b s h p", b=batch, p=headdim)
            dx, _ddt, dA, dB, dC, dD, _, ddt_bias, dinitial_states = _mamba_chunk_scan_combined_bwd(
                doubt,
                x,
                dt,
                A,
...
def mamba_split_conv1d_scan_combined(
    ...
    """Argument: zxbcdt: (batch, seqlen, 2 * dim + 2 * ngroups * dstate + nheads) where dim == nheads * headdim
    conv1d_weight: (dim + 2 * ngroups * dstate, width) conv1d_bias: (dim + 2 * ngroups * dstate,) dt_bias: (nheads,)
    A: (nheads) D: (nheads, headdim) or (nheads,) initial_states: (batch, nheads, headdim, dstate) seq_idx: (batch,
    seqlen), int32 rmsnorm_weight: (dim,) outproj_weight: (out_dim, dim) outproj_bias: (out_dim,) headdim: if D is
    1D, headdim must be passed in norm_before_gate: if True, we do RMSNorm(x) * F.silu(z). If False, we do RMSNorm(x
    * F.silu(z)).
...
def mamba_split_conv1d_scan_ref(
    ...
    """Argument: zxbcdt: (batch, seqlen, 2 * dim + 2 * ngroups * dstate + nheads) where dim == nheads * headdim
    conv1d_weight: (dim + 2 * ngroups * dstate, width) conv1d_bias: (dim + 2 * ngroups * dstate,) dt_bias: (nheads,)
    A: (nheads) D: (nheads, headdim) or (nheads,) rmsnorm_weight: (dim,) outproj_weight: (out_dim, dim)
    outproj_bias: (out_dim,) headdim: if D is 1D, headdim must be passed in norm_before_gate: if True, we do
    RMSNorm(x) * F.silu(z). If False, we do RMSNorm(x * F.silu(z)).
