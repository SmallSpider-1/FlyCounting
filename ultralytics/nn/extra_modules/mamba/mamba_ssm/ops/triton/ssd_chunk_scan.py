    ...
    if IS_TRITON_22 or pid_c > -1:
        ...
        if BLOCK_SIZE_DSTATE <= 128:
            ...
            for k in range(0, dstate, BLOCK_SIZE_K):
                C = tl.load(
                    C_ptrs, mask=(offs_m[:, None] < chunk_size_limit) & (offs_k_dstate[None, :] < dstate - k), other=0.0
                )
...
def _chunk_scan_bwd_dz_kernel(
    ...
    chunk_size_limit = min(chunk_size, seqlen - pid_c * chunk_size)
    doubt = tl.load(dout_ptrs, mask=(offs_m[:, None] < chunk_size_limit) & (offs_n[None, :] < hdim), other=0.0).to(
        tl.float32
    )
    ...
    if HAS_D:
        ...
        out -= x * D
    if HAS_DDACS:
        ddA_cs = tl.sum(doubt * out, axis=1)
...
def _chunk_scan_bwd_dstates_kernel(
    ...
    for k in range(0, chunk_size_limit, BLOCK_SIZE_K):
        ...
        doubt = (doubt * scale_k).to(dout_ptr.dtype.element_ty)
...
def _chunk_scan_bwd_dc_kernel(
    ...
    for h in range(nheads_iter):
        doubt = tl.load(dout_ptrs, mask=(offs_m[:, None] < chunk_size_limit) & (offs_k[None, :] < hdim), other=0.0)
...
def _chunk_scan_bwd_dx_kernel(
    ...
    #     doubt = tl.load(dout_new_ptrs, mask=(offs_m[:, None] < M) & (offs_n[None, :] < N), other=0.0).to(tl.float32)
    #     dD = tl.sum(x * doubt, axis=0)
...
def _chunk_scan_bwd_ddAcs_stable_kernel_old(
    ...
    #     doubt = tl.load(dout_ptrs, mask=(offs_m[:, None] < chunk_size_limit) & (offs_k[None, :] < hdim - k), other=0.0)
...
def _chunk_scan_bwd_dz(x, z, out, doubt, chunk_size, has_ddAcs=True, D=None, dz=None, recompute_output=False):
    batch, seqlen, nheads, headdim = x.shape
    assert z.shape == x.shape
    assert out.shape == x.shape
    assert doubt.shape == out.shape
...
def _chunk_scan_bwd_dstates(C, dA_cumsum, doubt, seq_idx=None, dtype=None):
    batch, seqlen, nheads, headdim = doubt.shape
...
def _chunk_scan_bwd_dC(prev_states, dA_cumsum, doubt, seq_idx=None, C=None, ngroups=1):
    batch, nchunks, nheads, headdim, dstate = prev_states.shape
    _, seqlen, _, _ = doubt.shape
    ...
    if C is not None:
        ...
        ddA_cumsum_prev = torch.empty(batch, nheads, nchunks, chunk_size, device=doubt.device, dtype=torch.float32)
    ...
    sm_count = torch.cuda.get_device_properties(doubt.device).multi_processor_count
    ...
    dC = torch.empty(batch, seqlen, nsplits, ngroups, dstate, device=doubt.device, dtype=torch.float32)
    ...
    with torch.cuda.device(doubt.device.index):
...
def _chunk_scan_bwd_dcb(x, dt, dA_cumsum, doubt, seq_idx=None, CB=None, ngroups=1):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    assert dt.shape == (batch, nheads, nchunks, chunk_size)
...
def _chunk_scan_bwd_dx(cb, x, dt, dA_cumsum, doubt, D=None):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    ngroups = cb.shape[2]
    assert nheads % ngroups == 0
...
def _chunk_scan_bwd_ddAcs_unstable(x, dt, out, doubt, ddt, D=None, subtract_ddtdt=True):
    """Not numerically stable and should not be used. Leaving here for reference."""
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    assert dt.shape == (batch, nheads, nchunks, chunk_size)
...
def _chunk_scan_bwd_ddAcs_stable_old(x, dt, dA_cumsum, doubt, cb):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    assert dt.shape == (batch, nheads, nchunks, chunk_size)
    assert doubt.shape == x.shape
...
def _chunk_scan_bwd_ddAcs_stable(x, dt, dA_cumsum, doubt, cb):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    assert dt.shape == (batch, nheads, nchunks, chunk_size)
    assert doubt.shape == x.shape
...
def _chunk_scan_bwd_ddAcs_prev(prev_states, C, doubt, dA_cumsum, seq_idx=None):
    batch, nchunks, nheads, headdim, dstate = prev_states.shape
    _, seqlen, _, _ = doubt.shape
    _, _, _, chunk_size = dA_cumsum.shape
    assert prev_states.shape == (batch, nchunks, nheads, headdim, dstate)
...
class ChunkScanFn(torch.autograd.Function):
    ...
    def backward(ctx, doubt):
        ...
        batch, seqlen, nheads, headdim = x.shape
        _, _, _nchunks, chunk_size = dt.shape
        _, _, ngroups, _dstate = B.shape
        assert doubt.shape == (batch, seqlen, nheads, headdim)
        if z is not None:
            dz, doubt, dD, ddA_cumsum = _chunk_scan_bwd_dz(x, z, out, doubt, chunk_size=chunk_size, D=D)
        else:
            dz = None
        dprev_states = _chunk_scan_bwd_dstates(C, dA_cumsum, doubt, dtype=prev_states.dtype)
        dC = _chunk_scan_bwd_dC(prev_states, dA_cumsum, doubt, ngroups=ngroups)
        dC = dC.to(C.dtype)
        dCB = _chunk_scan_bwd_dcb(x, dt, dA_cumsum, doubt, ngroups=ngroups)
        dCB = dCB.to(CB.dtype)
        dB = _bmm_chunk_bwd(C, dCB)
        dC = _bmm_chunk_bwd(B, rearrange(dCB, "... l s -> ... s l"), residual=dC)
        dx, ddt = _chunk_scan_bwd_dx(CB, x, dt, dA_cumsum, doubt, D=D)
        ...
        if z is not None:
            ...
            ddA_cumsum, dD = _chunk_scan_bwd_ddAcs_unstable(x, dt, out, doubt, ddt, D=D)
...
def chunk_scan(B, C, x, dt, dA_cumsum, prev_states, D=None, z=None):
    ...
    Argument: B: (batch, seqlen, ngroups, dstate) C: (batch, seqlen, ngroups, dstate) x: (batch, seqlen, nheads,
    headdim) dt: (batch, nheads, nchunks, chunk_size) dA_cumsum: (batch, nheads, nchunks, chunk_size) prev_states:
    (batch, nchunks, nheads, headdim, dstate) D: (nheads, headdim) or (nheads,) z: (batch, seqlen, nheads, headdim).
...
def chunk_scan_ref(B, C, x, dt, dA_cumsum, prev_states, D=None, z=None):
    ...
    Returns:
        out: (batch, seqlen, nheads, headdim).
    """
    batch, seqlen, nheads, _headdim = x.shape
    _, _, ngroups, dstate = B.shape
    assert B.shape == (batch, seqlen, ngroups, dstate)
    _, _, nchunks, chunk_size = dt.shape
    assert seqlen == nchunks * chunk_size
