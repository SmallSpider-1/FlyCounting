...
class ChunkStateFn(torch.autograd.Function):
    ...
    def backward(ctx, dstates):
        ...
        batch, _seqlen, nheads, headdim = x.shape
        _, _, nchunks, _chunk_size = dt.shape
        _, _, ngroups, dstate = B.shape
        assert dstates.shape == (batch, nchunks, nheads, headdim, dstate)
...
def chunk_state(B, x, dt, dA_cumsum, states_in_fp32=True):
    """Argument: B: (batch, seqlen, ngroups, headdim) x: (batch, seqlen, nheads, headdim) dt: (batch, nheads, nchunks,
    chunk_size) dA_cumsum: (batch, nheads, nchunks, chunk_size).
