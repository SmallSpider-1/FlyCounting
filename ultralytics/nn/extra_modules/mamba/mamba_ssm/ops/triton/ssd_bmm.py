...
def _bmm_chunk_fwd(a, b, chunk_size, seq_idx=None, causal=False, output_dtype=None):
    """Argument: a: (batch, seqlen, k) or (batch, seqlen, ngroups, k) b: (batch, seqlen, k) or (batch, seqlen, ngroups,
    k) seq_idx: (batch, seqlen) or None. out[i, j] for seq_idx[i] != seq_idx[j] will be zeroed out. causal: if True,
    then out[i, j] for i > j will be arbitrary, only out[i, j] for i <= j are guaranteed to be correct.
...
def _bmm_chunk_bwd(a, doubt, residual=None, out=None):
    """Argument: a: (batch, seqlen, k) or (batch, seqlen, ngroups, k) doubt: (batch, nchunks, chunk_size, chunk_size) or
    (batch, nchunks, ngroups, chunk_size, chunk_size) residual: (batch, seqlen, k) or (batch, seqlen, ngroups, k).

    Returns:
        out: (batch, seqlen, k) or (batch, seqlen, ngroups, k).

            If there was seq_idx in the fwd pass, then doubt[i, j] for seq_idx[i] != seq_idx[j] should already be zeroed
    ...
    if not has_groups:
        ...
        batch, seqlen, ngroups, k = a.shape
    nchunks, chunk_size = doubt.shape[1], doubt.shape[-1]
    if a.stride(-1) != 1 and a.stride(-2) != 1:
        a = a.contiguous()
    if doubt.stride(-1) != 1 and doubt.stride(-2) != 1:
    ...
    dot_dtype = (
        tl.bfloat16
        if a.dtype == torch.bfloat16 or doubt.dtype == torch.bfloat16
        else (tl.float16 if a.dtype == torch.float16 or doubt.dtype == torch.float16 else tl.float32)
    )
    ...
    with torch.cuda.device(a.device.index):
        _bmm_chunk_bwd_kernel[grid](
            ...
            doubt.stride(0),
            doubt.stride(1),
            0 if not has_groups else doubt.stride(2),
            doubt.stride(-2),
            doubt.stride(-1),
            out.stride(0),
            out.stride(1),
