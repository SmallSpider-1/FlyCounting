...
class VSSM(nn.Module):
    ...
    def _make_patch_embed(
        in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False
    ):
    ...
    def flops(self, shape=(3, 224, 224), verbose=True):
        ...
        Gflops, _unsupported = flop_count(model=model, inputs=(input,), supported_ops=supported_ops)
    ...
    def _load_from_state_dict(
        ...
        if check_name("pos_embed", strict=True):
            ...
            state_dict[prefix + "pos_embed"] = F.interpolate(
                srcEmb.float(), size=self.pos_embed.shape[2:4], align_corners=False, mode="bicubic"
            ).to(srcEmb.device)
