...
for _name in (
    ...
    globals().setdefault(_name, _MissingCustomModule)
        ...
        for m in self.model:
            ...
            if hasattr(m, "backbone"):
                ...
                for _ in range(5 - len(x)):
                    x.insert(0, None)
                for i_idx, output in enumerate(x):
                    y.append(output if i_idx in self.save else None)
                x = x[-1]
            else:
                x = m(x)  # run
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    # Args
    legacy = True  # backward compatibility for v3/v5/v8/v9/v11-style heads
    max_channels = float("inf")
    nc, act, scales, end2end = (d.get(x) for x in ("nc", "activation", "scales", "end2end"))
    reg_max = d.get("reg_max", 16)
    threshold = d.get("threshold", 0.25)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        try:
            ...
            if m is _MissingCustomModule:
                raise ImportError(
                    f"Unable to load migrated YOLO26 module '{t}'. The optional YOLO11 improvement modules failed "
                    f"to import first: {CUSTOM_MODULE_IMPORT_ERROR}"
                )
        except Exception as e:
            if isinstance(m, str) and CUSTOM_MODULE_IMPORT_ERROR is not None:
                raise ImportError(
                    f"Unable to load migrated YOLO26 module '{m}'. The optional YOLO11 improvement modules failed "
                    f"to import first: {CUSTOM_MODULE_IMPORT_ERROR}"
                ) from e
            pass
